...
func callback(target: SCNetworkReachability, flags: SCNetworkReachabilityFlags, info: UnsafeMutableRawPointer?) {
    ...
    let reachability = Unmanaged<SwiftReachability>.fromOpaque(UnsafeRawPointer(OpaquePointer(info))!).takeUnretainedValue()
...
open class SwiftReachability {
    ...
    // MARK: Singleton
    static let sharedManager: SwiftReachability? = SwiftReachability.managerForAddress(SwiftReachability.zeroAdress)
    ...
    class func managerForDomain(_ domain: String) -> SwiftReachability? {
        ...
        return SwiftReachability(reachabilityRef: reachabilityRef!, reachabilityAssociation: .forName)
    }
    
    class func managerForAddress(_ address: sockaddr_in) -> SwiftReachability? {
        ...
        return SwiftReachability(reachabilityRef: reachabilityRef! as SCNetworkReachability, reachabilityAssociation: .forAddress)
    }
    ...
    func startMonitoring() {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                NotificationCenter.default.post(name: Notification.Name(rawValue: ReachabilityDidChangeNotification), object:self, userInfo: [ReachabilityNotificationStatusItem : "\(status)"])
            }
        }
    }
